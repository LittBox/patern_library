function buildFallback(input = {}) {
  const theme = input.theme || '民族纹案'
  const ethnicStyle = input.ethnicStyle || '民族传统'
  const palette = input.palette || '综合色彩'
  const composition = input.composition || '对称构图'
  const usage = input.usage || '文创展示'
...
export async function generateMeta(input = {}) {
  ...
  if (!apiKey) {
    return {
      ...
      fallback: true,
      message: '未配置 DEEPSEEK_API_KEY，已使用本地模板。'
    }
  }
...
${JSON.stringify(input, null, 2)}
  ...
  if (!response.ok) {
    ...
    throw new Error(`DeepSeek 请求失败: ${response.status} ${text}`.trim())
  }
  ...
  return {
    data: { ...parsed, textModel: 'deepseek-chat' },
    fallback: false,
    message: ''
  }
}
